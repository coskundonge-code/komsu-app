...
const mockNotifications: Notification[] = [
  ...
  },
  {
    ...
    userName: "Ayşe Kılıç",
    action: "Satış için kouduğunuz bisikleti satın almak istiyor",
    timestamp: "6 saat",
    read: false,
  ...
  },
  {
    ...
    category: "comments",
    userName: "Hasan Köse",
    action: "Komşu Kahvaltısı etkinliğinbn gönderiyle ilgili yorum yaptı",
    timestamp: "2 gün",
    read: true,
...
export default function NotificationsPage() {
  ...
  const handleMarkAllAsRead = async () => {
    if (!user) return;
    const { error } = await markAllNotificationsAsRead(uert":
      return <AlertCircle className="w-5 h-5 text-orange-500" />;
    case "marketplace":
      return <ShoppingBag className="w-5 h-5 text-primary" />;
    case "event":
