        ...
        <>
          ...
          {showComment && (
            <div className="px-4 py-3 border-t border-border-light bg-surface-hover/30">
              <div className="flex items-start gap-3">
                <div class="w-8 h-8 bg-primary rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0">{\profile?.full_name?.[0]?.toUpperCase() || 'K'}</div>
                <div className="flex-1">
                  ...
                  <div className="flex gap-2 mt-2 justify-end">
                    <button onClick={() => { setShowComment(false); setCommentText('') }} class="p-1.5 text-text-muted hover:bg-surface-active rounded-lg">
                      <X class="w-4 h-4" />
                    </button>
                    <button
